...
format_time() {
  ...
  local msec=$(($1%1000))
  if [ "$1" -lt 10000 ]; then
    echo "%F{green}%B$1%b ms.%f"
  elif [ "$1" -le 60000 ]; then
    echo "%F{green}%B$sec%b s. %B$msec%b ms.%f"
  elif [ "$1" -gt 60000 ] && [ "$1" -le 180000 ]; then
    echo "%F{yellow}%B$min%b min. %B$sec%b s.%f"
  else
    if [ "$hours" -gt 0 ]; then
      min=$(($min%60))
      echo "%F{red}%B$hours%b h. %B$min%b min. %B$sec%b s.%f"
    else
      echo "%F{red}%B$min%b min. %B$sec%b s.%f"
    fi
  fi
}
